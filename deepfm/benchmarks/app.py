...
def transform_data(user_input):
    ...
    df_inference = pd.read_csv(os.path.join(params['data_root'], "all_business.csv"))
    ...
    for col_name, col_val in user_input.items():
        df_inference[col_name] = col_val
    df_inference["target"] = 0 # a redundant value in order to use the framework
    df_inference.to_csv(params['test_data'], index=False)
...
def postprocess_prediction(inference_result):
    ...
    return df_inference[["business_name", "categories", "score"]].to_dict('records')


@app.route('/', methods=['GET'])
def root():
    return jsonify({'msg': 'Try POSTing to the /predict endpoint'})
...
def predict():
    if request.method == 'POST':
        ...
        if user_input is not None:
            inference_gen = transform_data(user_input)
            inference_result = get_prediction(inference_gen)
            recommendations = postprocess_prediction(inference_result)
